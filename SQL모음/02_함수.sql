...
-- 함수는 SELECT문의 
-- SELECT 절, WHERE 절, ORDER BY절, GROUP BY절, HAVING절 사용 가능 
...
-- 문자열을 5번째 문자부터 검색하여 첫 번째 B위치를 조회 

SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL; 

-- 문자열을 5번째 문자부터 검색하여 두 번째 B위치를 조회 
...
-- 2024-02-29 11:20:37.928 +0900 (UTC에서 +9시간 나타냄, 한국 표준시)
...
FROM EMPLOYEE ORDER BY EMP_NAME, HIRE_DATE;
...
SELECT TO_CHAR(1234) FROM DUAL; -- 컬럼에 값이 있을 경우 1,234 -> '1234'로 변환  
...
-- NVL(컬럼명, 컬람값 NULL일 때 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경
...
-- 급여 최소값, 가장 빠른 입사일, 알파벳 순서가 가장빠른 이메일을 조회 
...
SELECT SALARY,EMP_NAME ,JOB_CODE  
FROM EMPLOYEE
...
SELECT COUNT(*)  
FROM EMPLOYEE; -- 23행, EMPLOYEE 테이블의 행의 개수 
